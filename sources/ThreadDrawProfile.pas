...
type
  TDrawThreadProfile = class(TThread)
  private
...
var  ProfileThread:TDrawThreadProfile;
     ProfileThreadActive:boolean;
implementation
...
uses    CspmVar,GlobalVar, GlobalDCl,GlobalFunction, SioCSPM,frProfiler;

procedure TDrawThreadProfile.Execute;
...
//         end;
   ...
   if  (NewPCount>0) then
    ...
    PostMessage(ProfilerWnd.Handle,wm_ThreadDoneMsg,mScanning,0);
    ScanDone;
    FreeChannels;
  end; //channel out
 end;  //create channels
...
procedure TDrawThreadProfile.GetLineData;
...
//  if FlgCurrentUserLevel<>Demo then dataoutcount:=0;
  ...
  begin
     valueZ:=datatype(PIntegerArray(DataBuf)[i+dataoutcount]  shr 16);
     valueX:=datatype(PIntegerArray(DataBuf)[i+1+dataoutcount] shr 16);
     dir:= datatype(PIntegerArray(DataBuf)[i+2+dataoutcount]);
     ScanData.AquiSpectr.Data[SpectrParams.CurrentLine,kt]:=valueZ;       //discrets
     ScanData.AquiSpectr.Data[SpectrParams.CurrentLine,kt+1]:= valueX;
     TempAquiData[i]:=valueZ;
     TempAquiData[i+1]:=valueX;
...
procedure TDrawThreadProfile.DrawCurrentLine;
var dir,i,k:integer;
value,z:single;
umstart:single;
begin
(*  umstart:=round( ScanData.AquiSpectr.Data[SpectrParams.CurrentLine,1]*SpectrWNd.ZStepMin);;
 ...
 while i<3*n do
  begin
     value:=TempAquiData[i]*ProfilerWnd.StepValueMin;
     Z:=TempAquiData[i+1]*ProfilerWnd.ZStepMin;
     dir:=TempAquiData[i+2];
     ProfilerWnd.ArChart.HeadSeriesList.Series.AddXY(Z,Value);
...
constructor TDrawThreadProfile.Create;
begin
...
destructor TDrawThreadProfile.Destroy;
begin
   ThreadFlg:=mDrawing;
   PostMessage(ProfilerWND.Handle,wm_ThreadDoneMsg,ThreadFlg,0);
   inherited destroy;
end;
end.
