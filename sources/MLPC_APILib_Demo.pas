...
var i,j,k:integer;
 ...
 begin
  ...
  if  ApprOnProgr   then                      // открыто окно ѕодвода
   begin
        if  (((ApproachParams.ZStepsDone) < 0)    // «онд выше зоны, в которой измен€етс€ его длина
          or (ApproachParams.ZStepsNumb < 0))       //или  выполн€етс€ отвод
         ...
         if  (ApproachParams.ZStepsNumb > 0) and // ѕодвод при уже захваченном взаимодействии
              (abs(DemoParams.Z) < abs(ZGatediscr_min)) and (abs(DemoParams.Z) < abs(ZGatediscr_max))  then
         begin
              for i := 0 to pCount - 1 do
                begin
