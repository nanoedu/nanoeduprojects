...
type
 ...
 TfrmMaterialOption = class(TForm)
    Panel1: TPanel;
    material: TComboBox;
...
var
  frmMaterialOption: frmMaterialOption;
...
procedure   TfrmMaterialOption.InitMaterials;
const
...
procedure TfrmMaterialOption.FormPaint(Sender: TObject);
var ps:PAINTSTRUCT;
begin
 DC:=GetDC(Handle);
...
procedure TfrmMaterialOption.FormCreate(Sender: TObject);
begin
 width:=height;
 DC := GetDC (Handle);
 SetDCPixelFormat(DC);
...
procedure TfrmMaterialOption.FormDestroy(Sender: TObject);
begin
// wglMakeCurrent(0, 0);
...
procedure TfrmMaterialOption.FormResize(Sender: TObject);
...
procedure TfrmMaterialOption.FormClose(Sender: TObject; var Action: TCloseAction);
begin
   Action:=cafree;
end;

procedure TfrmMaterialOption.materialSelect(Sender: TObject);
begin
        with Material do
  begin
  case  ItemIndex of
