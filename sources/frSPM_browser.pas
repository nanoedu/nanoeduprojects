...
uses
  ...
  Menus, StdCtrls, FileCtrl, Tabs, ComCtrls, Grids,ExtCtrls, ExtDlgs,jpeg,
  spm_browserclasses, inifiles,cspmvar, siComp, siLngLnk,NTShellCtrls, ToolWin,
  siDialog;
type
  TfrSPMView = class(TForm)
    ...
    ToolBarBrowser: TToolBar;
    ToolBtnSaveAs: TToolButton;

    ToolBtnDel: TToolButton;
    siSaveDialog: TSaveDialog;
    ...
    procedure NTShellListViewKeyDown(Sender: TObject; var Key: Word;  Shift: TShiftState);
    procedure NTShellListViewChange(Sender: TObject; Item: TListItem; Change: TItemChange);
...
uses  math,GlobalVar,mMain,uFileOp,frNoFormUnitLoc, RenishawVars;
{$R *.DFM}
...
procedure TfrSPMView.ToolBtnSaveAsClick(Sender: TObject);
 var
  ...
  filtind:integer;
begin
   siSaveDialog.InitialDir:=saveasdirectory;
   ...
   if  stm_active.stm_label.caption<>'' then
      begin
       ...
       begin
         ...
         if assigned(WorkView) then begin ReDraw; end;
         Application.processmessages;
...
procedure TfrSPMView.ToolBtnDelClick(Sender: TObject);
var
  InFileName:string;
 begin   //delete
       if assigned(Stm_Active)  then             //31.10.16
       begin
             ...
             if silanglinked1.MessageDlg(siLangLinked1.GetTextOrDefault('IDS_4' (* 'Do you really want to delete ' *) ) +' '+ ExtractFileName(INFileName) + ' ?', mtConfirmation, [mbYes, mbNo], 0) = IDYes then
              ...
              //update all form stm_view
               if assigned(DirView) then
                  if (workdirectory=DirView.NTShellTreeView.Path) then   DirView.Redraw
              end;
            end;
           end;
//           end;
