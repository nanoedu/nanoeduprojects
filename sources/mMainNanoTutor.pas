...
//    procedure  WMDDeviceChange(var MSG:TMessage);message WM_DEVICECHANGE;
  ...
  public
    ...
    procedure CreateMDIChild(Sender: TObject;const FileName:string;
              FlgView:integer;flgScan:boolean; flgRenishawCorrect:boolean); //flgScan =true -scan create during scanning; false open saved scan
...
begin
  ...
  begin
   ...
   fileName:=InitialDir+'\'+tutanima;//'\3D_Модель_NanoEducator_LE.mp4'
  ...
  begin
   locdir:='eng';
   InitialDir:=CommonNanoeduDocumentsPath+'video' +'\'+locdir;
   fileName:=InitialDir+'\'+tutanima;//'\3D_Model_NanoEducator_LE.mp4'
  end;
 if Fileexists(Filename) then   ExecAndWaitMainVideo(Filename,'','',SW_showNORMAL)
                         else   silang1.MessageDlg(Filename+strfilenotexists,mtWarning ,[mbOK],0);
...
//      ChooseWorkDir;
  ...
  false: if not  SFMExecute then
     ...
     begin
      ...
      lFlgStatusStep:=NanoEdu.RisingToStartPoint(30);   //091112
   ...
   begin
        ...
        PositionXY.Visible:=(flgUnit=sem);
        Application.ProcessMessages;
      case STMFLG  of
  false: begin
  ...
  True: begin
        ...
        case FlgUnit of
 SEM,Baby:  Training.Visible:=false;
        end;
...
begin
   UpdateStrings;
    basecaption:=strm11{'NanoeducatorLE;'};
...
begin
  ...
  ie := CreateOleObject('InternetExplorer.Application') as IWebBrowser2;
   case lang of
 2: URL:='http://www.ntmdt.ru/online';
 1: URL:='http://www.ntmdt.com/online';
  end;
...
begin
 ...
 if not assigned(Formlog)       then  FormLog:=TFormlog.Create(self);
    ...
    basecaption:=strm11{'Nanoeducator;'};
    baseworkdircaption:=strm12{' Work Directory - '};
    addcaption:='';
    caption:=basecaption;
  ...
  grand: if fileexists(ExeFilePath+'Data\'+'nanoeduunit.jpg') then  UnitImage.Picture.LoadFromFile(ExeFilePath+'Data\'+'nanoeduunit.jpg');
    sem: if fileexists(ExeFilePath+'Data\'+'sem.jpg')         then  UnitImage.Picture.LoadFromFile(ExeFilePath+'Data\'+'sem.jpg');
...
//       firsttime:=0;
     ...
     if not DirectoryExists(OpenDirectory) then  OpenDirectory:=UserNanoeduWorkDocumentsPath;//ExeFilePath+'work';
      LithoTemplDirectory:=ReadString('Files','Litho Template Directory', ScanGalleryDir+'\litho');
     if not DirectoryExists(LithoTemplDirectory) then  LithoTemplDirectory:=ScanGalleryDir+'\litho';
  end;
 finally
     iniCSPM.Free;
 end;
...
procedure TMain.CreateMDIChild(Sender: TObject;const FileName: string;
                                  FlgView:integer;flgScan:boolean; flgRenishawCorrect:boolean );
var
  ...
  (* if   EXE_Running(FirmCameraWinName{'CTLVCentral3.exe'}, false) then
   ...
   with StartInfo do
    begin //cb := SizeOf(SUInfo);
     ...
     Result := CreateProcess(nil, PChar( String( CmdLine ) ), nil, nil, false,CREATE_NEW_CONSOLE or NORMAL_PRIORITY_CLASS, nil, PChar(ExtractFilePath(FirmCameraSoftPath)),StartInfo,ProcInfo);
...
begin
 flgCanClose:=false;
  ...
  Scanparams.DiscrNumInMicroStep:=10;
   NanoEdu.ScanMoveToX0Y0Method( XStartnm, YStartnm);
...
*)
  ...
  if FileExists(fileName) then  res:= ShellExecute(handle,nil,Pchar(fileName),nil,nil,SW_RESTORE)
                          else  silang1.MessageDlg(Filename+strfilenotexists,mtWarning ,[mbOK],0);
    ...
    if GetModuleHandle('MSVideoLib.dll')=0 then
     begin
      ...
      LibVideoHandle:=LoadLibrary(PChar(ExeFilePath+'MSVideoLib.dll'));
       ...
       StartVideo(Application.Handle,self.Handle,WM_UserCloseVideo,Lang);
       h:=findwindow(nil,Pchar(strm41{'MSVideo'}));
      if h<>0  then
       begin
        GetWindowRect(h,R);
