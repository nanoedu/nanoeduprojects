...
// if flgCurrentUserLevel=Demo then  j:=n;
 if (ScanParams.ScanPath<>Multi) and (ScanParams.ScanPath<>MultiY) then CurScan:=ScanParams.CurrentScanCount                                                                  else CurScan:=0;
 for i:=0 to lsz-1 do
  begin
...
if CreateChannels(AlgParams.NChannels) then
 ...
 while (not Terminated) and (CurrentP<PointsNmb) and (not flgEnd) do
  begin
    ...
    if ntoread >= ScanParams.ScanPoints +mod512corr then         // +1 - лишняя точка, чтобы исключить передачу данных длиной,
                                             //  кратной 512 байт
     nhasread:=0;
     begin
     ...
     begin
      if (XPos=0){ and ((i<>0))}then
       ...
       nread:=ScanParams.ScanPoints+mod512corr;
      hr:=(arPCChannel[ch_Data_out].ChannelRead).Read(DataBuf,nread);    /// read data //nread is not number of elements ????
    (*  hasRead_nmb:=0;
      left_toRead:=nread;                                                 // закомментировано для рисования бликования
      while ( hasRead_nmb < nread) do
         begin
             nread:= left_toRead;
             hr:=(arPCChannel[ch_Data_out].Main as IMLPCChannelRead2).ReadWait(DataBuf,nread,100);
             hasRead_nmb := nread;
             left_toRead:=left_toRead-  hasRead_nmb;
             synchronize(DrawVirtualline);
         end;   *)
      {$IFDEF DEBUG}
       if Failed(hr) then Formlog.memolog.Lines.add(ScanWnd.siLangLinked1.GetTextOrDefault('IDS_36' (* 'error read channel data ' *) )+inttostr(nread)+ScanWnd.siLangLinked1.GetTextOrDefault('IDS_5' (* 'hr=' *) )+inttostr(hr))
                     else Formlog.memolog.Lines.add(ScanWnd.siLangLinked1.GetTextOrDefault('IDS_39' (* 'scandraw data has read ' *) )+inttostr(nread));
      {$ENDIF}
        if Failed(hr) then
                    begin
                      nread:=0;
