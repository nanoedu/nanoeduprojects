package mlab;  //anodelitho 261012

public class Lithoanode
{
...
for (i=0;i<12; i++)  { Simple.DumpInt(datain[i]);}
		...
		buf_params=new int[2];
                buf_params[0]=datain[i0+9]  ; //<<   ???    // amplifier
                buf_params[1]=datain[i0+10];                  // dt
                 wr=0;
                for (;  wr == 0; )
		{
   ...
   //                      Simple.DumpInt(dt);
                       ...
                       //      if (DacU!=0)
                                {
                               	 scene.startLoad( handle, PORT_U, point );
