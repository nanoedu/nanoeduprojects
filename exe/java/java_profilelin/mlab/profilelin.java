...
Simple.DumpInt(0xCCCAAAAA);
		...
		for(lines=slowlines; lines>0; --lines)
		{
			...
			dataout[fastlines*SZ] = 0;
                	...
                	wr=0;  rd=0;
			int s = fastlines +1;//  +1 - лишняя точка, чтобы исключить передачу данных длиной,
                                             //  кратной 512 байт
          ...
          switch (flgMotor)
{
 ...
 case 2:
			   ...
			   {if (Simple.bramRead(M_smX_status)== 0){wr=1;}}
                           CurPosMotorX = CurPosMotorX |  maskMotorOn;          // включение мотора
 ...
 case 3:
			   ...
			   {if (Simple.bramRead(M_smY_status)== 0){wr=1;}}
                           CurPosMotorY = CurPosMotorY |  maskMotorOn;          // включение мотора
