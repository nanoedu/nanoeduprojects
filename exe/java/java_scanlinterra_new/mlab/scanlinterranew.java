// 05/06/15 add +1 to dataout buffer
package mlab;     //05/02/13  // terra  filter IT ??
		//21.02.13  // Get Linear Steps by Channel

public class  scanlinterranew
{
...
//for (i=0;i<10; i++)  { Simple.DumpInt(datain[i]);}
		...
		JVIO stream_ch_data_out  = new JVIO(CH_DATA_OUT,SZ,fastlines*slowlines+slowlines,JVIO.FIFO,fastlines, 0); // 2
                JVIO stream_ch_params    = new JVIO(CH_PARAMS,  3, 1,JVIO.BUF,  1, 0);                       // 3
		JVIO stream_ch_linearsteps_in   = new JVIO(CH_LINEARSTEPS_IN ,slowlines+fastlines+1, 1,JVIO.FIFO, 1, 0);   //4
		...
		buf_params[2]=datain[i0+11];     // TimeITDelay
                 wr=0;
                for (;  wr == 0; )
		{
...
Simple.DumpInt(0xCCCAAAAA);
		...
		for(lines=slowlines; lines>0; --lines)
		{
			...
			stream_ch_data_out.Invalidate();
                        ...
                        if  (ScanMethod != OneLine)
                        {
		             if ( lines > 1 )   // Переход к следующей строке
                                  {
                                   if (  ScanPath == 0)
                                   ...
                                   if (  ScanPath == 0)  dacY -= (JMPY_SUM- JMPY[slowlines-1]);         //Возврат в начальную точку
