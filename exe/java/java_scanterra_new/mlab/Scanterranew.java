//corrected 050615 add +1 to dataout buffer
package mlab;     //05/02/13 //terra  filter IT ??

public class Scanterranew
{
...
//for (i=0;i<10; i++)  { Simple.DumpInt(datain[i]);}
		...
		{
                        fastlinescount=0;                                                                                              //   read buffers params
			rd=0;
		       for (;  rd == 0; )
			{
			...
			for(point=0; point<fastlines; point++)
			{
			  ...
			  dxchg.GetI( PORT_H   );
                          dxchg.GetI( PORT_PH  );
                          if (ScanMethod ==IT) {
						 dxchg.Wait(1000*TimeITDelay ); //ms
						 dxchg.GetI( PORT_I);
					       }
                          else                 { dxchg.GetI( PORT_ERR);}
                            ...
                            if (SZ==2)
                              {
                                ...
                                if ((ScanMethod == IT) | (ScanMethod == UAM))  {dataout[dst_i+1] =arr[src_i+2];}
                              }
			    if (SZ==1)	{dst_i += 1;}
                            else        {dst_i += 2;}
				src_i += 3;
