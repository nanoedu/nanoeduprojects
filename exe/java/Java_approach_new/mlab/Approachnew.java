...
//             for(i=0; i<10; i++) Simple.DumpInt(datain[i+i0]);
		...
		buf_status[0]=none;
	        buf_status[1]=(int)Simple.bramRead(M_PID_out);
                ...
                buf_status[3]=Simple.bramRead(M_smZ_status)>>16;
  ...
  //	  Simple.bramWrite(M_BASE_K, SET_POINT);
         ...
         // CurPosStepMotor = CurPosStepMotor + (1 << 15);  // Установка бита включения мотора
...
//  main cycle
           ...
           for(i=0; i<10; i++) Simple.DumpInt(buf_params[i]);
 ...
 // Simple.DumpInt(0xBBBBBBBB);
			  ...
			  lstatus = none;
			    if (FLG_CHECKTOP == 1) 
			  	...
			  	if ((idin & maskTopLimit) == 0)     // Проверка достижения крайнего верхнего положения
                                            { lstatus = outlimit;}
