...
{ 
    ...
    JVIO stream_apr_out =   new JVIO(CH_APR_OUT, 3, 1,JVIO.BUF,  3, 0);                        // 2
...
*/
                ...
                buf_STEP = new int[1];
                buf_STEP[0] =waitstep;
                wr = stream_steps.Write(buf_STEP, 1, 1000);
                int[] buf_STOP;
                buf_STOP = new int[1];
                buf_STOP[0] =0;
	 ...
	 process=none;  // stop
         status=none;
               ...
               while (buf_STOP[0] != MakeSTOP )
                  ...
                  wr = stream_steps.Read(buf_STEP, 1, 1000);

		  while ( buf_STEPS[0]=steps);
                  {
                   //   read buffers params
