...
//                twait           =    datain[i0+12];
		...
		JVIO stream_ch_data_out  = new JVIO(CH_DATA_OUT,1,(X_POINTS*Y_POINTS),JVIO.FIFO,fastlines, 0);  // 2
...
// Вектор перемещения и позиция точки
                code[5]   = 0x00000000 + (12<<16) + (0<<0);
		code[6]  = 0x20000000;
		// Контроль перемещения
                code[7]  = 0x00000000;
		code[8]  = 0x10000008;    //?????
// Считывание после паузы
                code[9] = 0x00000000 + (0<<16) + (17<<0);
                code[10] = 0x40000000 + pause - 1;
// Возврат
                code[11] = 0x80000000 + (5<<16) + 0;

//----- Список портов перемещения в точку ---------
		code[12] = 4;
		code[13] = Dxchg2.PORT_DIRY;
		code[14] = Dxchg2.PORT_DIRX;
		code[15] = Dxchg2.PORT_Y;
		code[16] = Dxchg2.PORT_X;
