package mlab;     //17.11.12  //oneline+; discrtinmicrostep
                 // 22/03/13  // additional element in buffer (  <> mod 512)

public class Scannew
{
	static int X_POINTS = 200;
	static int Y_POINTS = 200;
...
//for (i=0;i<10; i++)  { Simple.DumpInt(datain[i]);}
		...
		for(lines=slowlines; lines>0; --lines)
		{
			...
			for(point=0; point<fastlines; point++)
			{
			  ...
			  dxchg.GetI( PORT_H   );
                          dxchg.GetI( PORT_PH  );
                          if (ScanMethod == I) { dxchg.GetI( PORT_I);}
